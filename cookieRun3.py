...
def human_delay(min_val, max_val):
    ...
    """
    # Use Python's random.betavariate but randomize alpha/beta slightly each call
    # to vary the shape and evade simple bot-detection heuristics.
    # Keep a right-skew preference by favoring alpha < beta most of the time.
    if random.random() < 0.85:
        alpha = random.uniform(1.6, 2.6)
        beta = random.uniform(3.5, 6.5)
    else:
        # occasionally use a less-skewed or even left-skewed sample
        alpha = random.uniform(0.9, 3.5)
        beta = random.uniform(0.9, 4.5)

    skewed_factor = random.betavariate(alpha, beta)
    # safety clamp
    skewed_factor = max(0.0, min(1.0, skewed_factor))

    return min_val + (max_val - min_val) * skewed_factor


class FarmCoin3(Action):
    def __init__(self):
        super().__init__()
